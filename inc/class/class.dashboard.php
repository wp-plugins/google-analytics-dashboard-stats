...
class GADS_STATS_Dashboard {
    ...
    public function __construct($in_dir) {
		self::$instance    = $this;
		$this->ga_email    = get_option('gads_email');
	    $this->ga_password = get_option('gads_password');
		$this->ga_title    = get_option('gads_prop_label');
		$this->ga_id       = get_option('gads_prop_id');
	...
	public function cc_count_stats($profile_id,$title,$start_date,$end_date,$version){
			...
			$att_b = '';
			if (get_option('gads_sources') != 'false' || get_option('gads_content') != 'false') {
			        if (get_option('gads_sources') != 'false' && get_option('gads_content') != 'false') {
