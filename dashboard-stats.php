...
class MM_STATS_Functions {
	...
	public function checkSettings(){
		...
		if(get_option('mm_ga_stats_password') == ''){
			$errors[] = 'Analytics Password';
			$check    = true;
		}
			
		if(get_option('mm_ga_stats_prop_id') == ''){
			$errors[] = 'Property ID';
			$check    = true;
		}
			
	    if(get_option('mm_ga_stats_prop_label') == ''){
		...
		$message = 'Please setup the following to use Google Analytics Dashboard Stats: '.$list.'. <a href="options-general.php?page=mm-ga-stats-options">Visit Settings Page</a>';
    ...
    public function my_custom_dashboard_widgets() {
	    add_action('admin_enqueue_scripts', array($this,'ann_style'));
